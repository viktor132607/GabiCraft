...
export default function AboutPage() {
  const rows = [
    ...
    ["Галерия", "Подбрани авторски творби, представени с фокус върху композицията, цвета и характера на всяка работа."],
  ];

  return (
    <>
      <StructuredData data={aboutStructuredData} />
      <section aria-labelledby="about-heading" className="min-h-[68vh] bg-[#fffdf7] py-[92px] pt-[104px] max-[820px]:min-h-0 max-[820px]:pt-[76px] max-[620px]:py-[54px]">
      <div className="mx-auto w-[min(820px,calc(100%_-_40px))] max-[620px]:w-[min(820px,calc(100%_-_28px))]">
        ...
        <h1 id="about-heading" className="mb-6 font-serif text-[clamp(52px,7vw,88px)] leading-[0.98] tracking-[-0.04em] text-[#2d2a22] max-[620px]:text-[clamp(42px,13vw,62px)]">GabiCraft</h1>
        ...
        <dl className="mt-[52px] border-t border-[#e4d9c7] max-[620px]:mt-9">
          {rows.map(([label, copy]) => (
            <div key={label} className="grid grid-cols-[180px_minmax(0,1fr)] gap-7 border-b border-[#e4d9c7] py-6 max-[820px]:grid-cols-1 max-[820px]:gap-2">
              ...
              <dd className="m-0 leading-[1.7] text-[#756d61]">{copy}</dd>
            </div>
          ))}
        </dl>
      </div>
